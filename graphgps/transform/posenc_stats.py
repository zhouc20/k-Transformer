...
def compute_posenc_stats(data, pe_types, is_undirected, cfg):
    ...
    # Verify PE types.
    for t in pe_types:
        if t not in ['LapPE', 'EquivStableLapPE', 'SignNet', 'StableExpressivePE', 'RWSE', 'HKdiagSE', 'HKfullPE', 'ElstaticSE',
    ...
    if 'SignNet' in pe_types:
        ...
        data.eigvals_sn, data.eigvecs_sn = get_lap_decomp_stats(
            ...
            eigvec_norm=cfg.posenc_SignNet.eigen.eigvec_norm, stable=False)

    if 'StableExpressivePE' in pe_types:
        # Eigen-decomposition with numpy for SignNet.
        norm_type = cfg.posenc_StableExpressivePE.eigen.laplacian_norm.lower()
        if norm_type == 'none':
            norm_type = None
        L = to_scipy_sparse_matrix(
            *get_laplacian(undir_edge_index, normalization=norm_type,
        ...
        data.eigvals_sn, data.eigvecs_sn = get_lap_decomp_stats(
            evals=evals_sn, evects=evects_sn,
            max_freqs=cfg.posenc_StableExpressivePE.eigen.max_freqs,
            eigvec_norm=cfg.posenc_StableExpressivePE.eigen.eigvec_norm, stable=True)
    ...
    if 'EdgeRWSE' in pe_types:
        ...
        internal_directed = cfg.posenc_EdgeRWSE.internal_directed
...
def get_lap_decomp_stats(evals, evects, max_freqs, eigvec_norm='L2', stable=False):
    ...
    if stable:
        ...
        value = float('nan')
    if N < max_freqs:
        EigVecs = F.pad(evects, (0, max_freqs - N), value=value)
    else:
        EigVecs = evects

    # Pad and save eigenvalues.
    if N < max_freqs:
        EigVals = F.pad(evals, (0, max_freqs - N), value=value).unsqueeze(0)
    else:
