...
class ExphormerAttention(nn.Module):
    """
        Exphormer Layer in Exphormer paper.
        Supports our sparse implementation of high-order transformers with preprocessing.
    """

    def __init__(self, in_dim, out_dim, num_heads, use_bias, dim_edge=None, use_virt_nodes=False, use_local_neighbors=False):
        super().__init__()
        ...
        self.use_local_neighbors = use_local_neighbors
    ...
    def forward(self, batch):
        edge_attr = batch.get("local_neighbor_edge_attr", None) if self.use_local_neighbors else None
        edge_index = batch.get("local_neighbor_edge_index", None) if self.use_local_neighbors else None
        h = batch.x
        ...
        if self.use_virt_nodes:
            ...
            if edge_index is not None:
                ...
                edge_index = batch.virt_edge_index
                edge_attr = batch.virt_edge_attr
