...
def set_cfg_posenc(cfg):
    ...
    cfg.posenc_StableExpressivePE = CN()
    cfg.posenc_RWSE = CN()
    cfg.posenc_HKdiagSE = CN()
    cfg.posenc_ElstaticSE = CN()
    cfg.posenc_EquivStableLapPE = CN()
    cfg.posenc_HodgeLap1PE = CN()
    cfg.posenc_EdgeRWSE = CN()
    cfg.posenc_RRWP = CN()
    cfg.posenc_InterRWSE = CN()
    cfg.posenc_RD = CN()
    ...
    for name in ['posenc_LapPE', 'posenc_SignNet', 'posenc_RRWP', 'posenc_StableExpressivePE',
                 'posenc_RWSE', 'posenc_HKdiagSE', 'posenc_ElstaticSE',
                 'posenc_HodgeLap1PE', 'posenc_EdgeRWSE', 'posenc_InterRWSE', 'posenc_ERN', 'posenc_ERE', 'posenc_RD']:
    ...
    for name in ['posenc_LapPE', 'posenc_SignNet', 'posenc_StableExpressivePE', 'posenc_EquivStableLapPE', 'posenc_HodgeLap1PE']:
        pecfg = getattr(cfg, name)
        pecfg.eigen = CN()
    ...
    for name in ['posenc_RWSE', 'posenc_HKdiagSE', 'posenc_ElstaticSE', 'posenc_EdgeRWSE', 'posenc_InterRWSE']:
        pecfg = getattr(cfg, name)
