...
from torch_geometric.graphgym.config import (cfg, dump_cfg,
                                             set_cfg, load_cfg,
                                             makedirs_rm_exist)
from torch_geometric.graphgym.loader import create_loader, create_dataset
...
if __name__ == '__main__':
    ...
    for run_id, seed, split_index in zip(*run_loop_settings()):
        ...
        train_loader = DataLoader(train_dataset, cfg.train.batch_size, shuffle=True, collate_fn=PyG_collate)
        val_loader = DataLoader(val_dataset, cfg.train.batch_size, shuffle=False, collate_fn=PyG_collate)
        test_loader = DataLoader(test_dataset, cfg.train.batch_size, shuffle=False, collate_fn=PyG_collate)
        loaders = [train_loader, val_loader, test_loader]
        cfg.share.num_splits = 3
        loggers = create_logger()
        model = create_model(dim_out=1)
        if cfg.pretrained.dir:
